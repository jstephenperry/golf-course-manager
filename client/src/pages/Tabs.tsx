...
export function Tabs() {
  const { data, tabs: tabsApi } = useStore();
  ...
  const createTab = async () => {
    if (newForm.memberIds.length === 0 && newForm.guests.length === 0) {
      toaster.push({
        kind: "error",
        message: "Add at least one member or guest.",
      });
      return;
    }
    setBusy(true);
  ...
  const addItem = async (
    ...
    await tabsApi.addItem(tab.id, product.id, qty, notes);
  };

  const removeItem = async (tab: PlayerTab, item: TabLineItem) => {
    if (!window.confirm(`Remove ${item.name}?`)) return;
    await tabsApi.removeItem(tab.id, item.id);
  };
  ...
  const changeItemQty = async (
    ...
    if (next < 1) return;
    await tabsApi.setItemQuantity(tab.id, item.id, next);
  };
  ...
  const addPayment = async (
    ...
    await tabsApi.addPayment(tab.id, payment);
  };

  const removePayment = async (tab: PlayerTab, payment: TabPayment) => {
    if (!window.confirm("Remove this payment?")) return;
    await tabsApi.removePayment(tab.id, payment.id);
  };

  const settleTab = async (tab: PlayerTab) => {
    const totals = tabTotals(tab);
    if (totals.balance > 0.005) {
      toaster.push({
        kind: "error",
        message: `Balance ${formatMoney(totals.balance)} still owed. Apply payment first.`,
      });
      return;
    }
    await tabsApi.settle(tab.id);
  };

  const reopenTab = async (tab: PlayerTab) => {
    await tabsApi.reopen(tab.id);
  };

  const voidTab = async (tab: PlayerTab) => {
    if (
      !window.confirm(
      ...
      return;
    await tabsApi.void(tab.id);
  };

  const updateTabMeta = async (id: string, patch: Partial<PlayerTab>) => {
    const current = data.tabs.find((t) => t.id === id);
    if (!current) return;
    await tabsApi.updateMeta(id, { ...current, ...patch });
  };
    ...
    <div className="stack">
      ...
      {activeTab && (
        <TabDetail
          ...
          updateMeta={(patch) => updateTabMeta(activeTab.id, patch)}
        />
      )}
      ...
      {creating && (
        <Modal
          ...
          onSubmit={createTab}
          submitLabel={busy ? "Opening…" : "Open tab"}
        >
          <div className="field">
...
interface TabDetailProps {
  ...
  allMembers: Member[];
  allProducts: Product[];
  addItem: (product: Product, qty: number, notes: string) => void | Promise<void>;
  removeItem: (item: TabLineItem) => void | Promise<void>;
  changeItemQty: (item: TabLineItem, delta: number) => void | Promise<void>;
  addPayment: (p: {
    method: PaymentMethod;
    amount: number;
    payerMemberId?: string;
  ...
  removePayment: (p: TabPayment) => void | Promise<void>;
  settle: () => void | Promise<void>;
  reopen: () => void | Promise<void>;
  voidTab: () => void | Promise<void>;
  updateMeta: (patch: Partial<PlayerTab>) => void | Promise<void>;
}
...
function TabDetail({
  ...
  const submitAdd = async () => {
    if (!product) {
      toaster.push({ kind: "error", message: "Pick a product." });
      return;
    }
    await addItem(product, pickerQty, pickerNotes);
    setPickerProductId("");
    setPickerQty(1);
    setPickerNotes("");
  };
  ...
  const submitPayment = async () => {
    ...
    if (!amount || amount <= 0) {
      toaster.push({ kind: "error", message: "Enter a positive amount." });
      return;
    }
    if (payMethod === "Member Charge" && !payer) {
      toaster.push({ kind: "error", message: "Pick which member to charge." });
      return;
    }
    await addPayment({
      method: payMethod,
