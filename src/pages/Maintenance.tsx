...
export function Maintenance() {
  ...
  const save = () => {
    ...
    if (editing) {
      update("maintenance", (list) =>
        list.map((m) => (m.id === editing.id ? { ...editing, ...form } : m)),
      );
      setEditing(null);
    } else {
      update("maintenance", (list) => [...list, { id: uid("mt"), ...form }]);
      setCreating(false);
    }
  };
  ...
  const remove = (id: string) => {
    ...
    update("maintenance", (list) => list.filter((m) => m.id !== id));
    setEditing(null);
  };

  const setStatus = (id: string, status: MaintenanceTask["status"]) => {
    update("maintenance", (list) =>
